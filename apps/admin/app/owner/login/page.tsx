...
export default function OwnerLoginPage() {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      const user = await api<{ role: string }>("/api/auth/internal/login", { method: "POST", body: JSON.stringify({ username, password }) });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6">
      <form onSubmit={onSubmit} className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="mb-6 text-xl font-bold text-slate-900">Signet Owner Sign In</h1>
        ...
        <button disabled={busy} className="w-full rounded bg-slate-900 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{busy ? "Signing in..." : "Sign in"}</button>
        <Link href="/login" className="mt-4 block text-center text-sm text-indigo-600 hover:underline">Back to Signet sign in</Link>
      </form>
    </div>
  );
}
