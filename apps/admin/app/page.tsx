  ...
  return (
    <Shell>
      <div className="mb-7 flex items-center gap-4">
        <span className="skeuo-orange grid h-20 w-20 place-items-center rounded-3xl text-4xl">◈</span>
        <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        <StatCard label="Client Sites" value={tenants?.length ?? "…"} href="/tenants" icon="♚" />
        <StatCard label="Total Orders" value={orders?.length ?? "…"} href="/orders" icon="🛒" />
        <StatCard
        ...
        />
        <StatCard label="Open Invoices" value={unpaidInvoices} href="/invoices" icon="▣" tone={pastDue > 0 ? "warn" : "default"} />
        <StatCard label="Quotes Awaiting Rep" value={openQuotes} href="/quotes" icon="◯" />
        <StatCard label="Open Returns" value={openReturns} href="/returns" icon="↩" />
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        <div className="skeuo-panel rounded-3xl p-5">
          <div className="mb-3 flex items-center justify-between"><h2 className="font-bold text-[color:var(--ink)]">▣ &nbsp;Client Sites</h2><Link href="/tenants" className="skeuo-orange rounded-xl px-4 py-1 text-xs font-bold">View all</Link></div>
